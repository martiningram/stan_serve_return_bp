data {
    ...
    // The surface to calculate relative to is coded as surface_id = 0.
    int surface_id[n_matches];
}
parameters {
    vector<lower=0>[n_surfaces - 1] tau_serve;
    vector<lower=0>[n_surfaces - 1] tau_return;

    matrix[n_surfaces - 1, n_players] z_serve_surf;
    matrix[n_surfaces - 1, n_players] z_return_surf;
    
    cholesky_factor_corr[n_surfaces - 1] L_Omega_serve;
    cholesky_factor_corr[n_surfaces - 1] L_Omega_return;
...
transformed parameters {
    matrix[n_players, n_surfaces - 1] beta_serve;
    matrix[n_players, n_surfaces - 1] beta_return;
...
model {
    ...
    for (i in 1:n_matches) {
        ...
        if (surface_id[i] == 0) {
          // We do not add any surface skills
          match_skills[i] = serve_skills[server_id[i]] - return_skills[returner_id[i]] 
                            + intercepts[surface_id[i] + 1];
        } else {
          // We add the relevant surface_skills
          match_skills[i] = serve_skills[server_id[i]] - return_skills[returner_id[i]] 
...
generated quantities {
    ...
    for (i in 1:n_players) {
        for (j in 0:n_surfaces - 1) {
            if (j == 0) {
              predicted_serve_probs[i, j + 1] = inv_logit(serve_skills[i] +
                intercepts[j + 1]);
              predicted_return_probs[i, j + 1] = 1 - inv_logit(intercepts[j + 1] -
                return_skills[i]);
            }
            else {
              predicted_serve_probs[i, j + 1] = inv_logit(serve_skills[i] +
                beta_serve[i, j] + intercepts[j + 1]);
              predicted_return_probs[i, j + 1] = 1 - inv_logit(intercepts[j + 1] -
                return_skills[i] - beta_return[i, j]);
            }
    }
    }
